...
2. Создать кортеж "aTuple". В нем сразу должны содержаться изменяемые и неизменяемые типы объектов, а так-же один индексируемый,итерируемый,неупорядоченные тип.Выведите на экран переменную "aTuple".
